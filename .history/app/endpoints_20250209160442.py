...
def get_router(model, tokenizer):
    ...
    async def stream_translation_chunks(content: str, instruction: str) -> AsyncGenerator[str, None]:
        for translated_chunk in translate_processing_with_streaming(model, tokenizer, instruction, content):
            yield translated_chunk

    @router.post("/api/v1/stream_translate")
    async def stream_translate(request: StreamTranslationRequest):
        """Streaming translate API"""
        instruction = "Translate with context" if request.field == "reportName" else "General translation"
        return StreamingResponse(
            stream_translation_chunks(request.content, instruction),
